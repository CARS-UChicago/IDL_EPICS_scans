...
pro xrd_expose,t=t, name=name, i0=i0, ext=ext
    ...
    x = caput('13SDD1:PresetReal', t)
    x = caput('13SDD1:EraseAll',      1)
    x = caput('13SDD1:PresetLive',    0)
    x = caput('13SDD1:PresetMode', 1)
    x = caput('13SDD1:CollectMode', 0)
    ...
    x = caput('13XRM:XRF:filebase', byte_name)

    if keyword_set(ext)  ne 0 then begin
       x = caput('13MAR345_1:cam1:FileNumber', ext)
    endif else begin
       x = caget('13MAR345_1:cam1:FileNumber_RBV', ext)
    endelse
    x = caput('13XRM:XRF:fileext', strtrim(string(ext),2))
    ...
    count = 0
    collecting = 1
    max_count  = t + 100  ; wait up to 100sec more than collection time
    wait, 2.0
    x = caput('13MAR345_1:cam1:Acquire', 1)
    x = caput('13XRM:XRF:Request', 1)
    x = caput('13BMA:OpenBMDShutter.PROC', 1)

    print , ' wait for detector: ' 
    wait, 10.0
    while collecting ne 0 do begin
       ...
       if collecting eq 2 then lower_shield
    endwhile
    lower_shield
    wait, 2.0

    ; turn autoshutter back to original state
